...
docker run --name ergo-miner-test-container -d --env-file .env.test ergo-miner-test
...
echo "Testing the health check..."
if ! docker inspect --format='{{.State.Health.Status}}' ergo-miner-test-container | grep -q "healthy"; then
  echo "Health check failed!"
  docker logs ergo-miner-test-container
...
if docker inspect -f '{{.State.Running}}' ergo-miner-test-container | grep -q "true"; then
    ...
    if ! echo "$METRICS_OUTPUT" | jq -e 'has("hashrate") and has("avg_temperature") and has("avg_fan_speed")' > /dev/null; then
